...
Concretely:
1. Remove `hibernate.connection.username`/`connection.password` from `src/main/resources/hibernate.cfg.xml` entirely — they currently hold the real, committed `mismo_web`/`cArn88r0w` credential (confirmed still there).
...
- Replace `create-user.sql` with a template containing **no real password** — a placeholder plus a one-line instruction to generate a fresh one per environment. The account itself (`mismo_web`, `GRANT ALL ... ON mismo.*`) still needs creating by hand on any new server; `mysqldump`/restore never carries user accounts or grants along with a single database's data.
...
| `src/db/local_database_refresh.sql` (new) | Rewrites production-specific values to their local-dev equivalents after a production snapshot is restored locally. For Mismo-Trainer, concretely: rewrites both `app_setting.hub.external.url` and `app_setting.mismo.external.url` back to their `localhost:8080` values — without this, a freshly-refreshed local dev database would have the InteropHub client pointed at *production's* Hub and think it's reachable at production's own URL, instead of the local ones. | Yes |
...
1. On the new server: create the MySQL 8.0 database and the `mismo_web` user (`create-user.sql`'s template, with a freshly generated password — set as that server's `MISMO_DB_PASSWORD`).
2. From here: `mysqldump -u<user> -p --single-transaction --no-tablespaces --set-gtid-purged=OFF --default-character-set=utf8mb4 mismo > mismo.sql`.
3. Transfer that file out-of-band (it is **not** checked into the project, per your direction) and restore it on the new server.
4. Apply `local_database_refresh.sql`-equivalent logic in the *other* direction here — i.e. make sure `app_setting.hub.external.url` and `app_setting.mismo.external.url` on production point at production's real InteropHub instance and this app's own production URL, not local dev's. Both are plain database rows — no JVM flags, env vars, or restarts involved in setting or later changing either one.
...
2. Drop and recreate the local `mismo` database.
...
- The local `mismo_web` MySQL account also had a stray `GRANT ALL ... ON aart.*` (a sibling project's database, not this one) alongside its legitimate `mismo` grant. With that in place, Hibernate 3.6's schema validator — which queries table metadata with a null catalog — non-deterministically inspected `aart`'s tables instead of this project's, producing false mismatch errors. Revoked; confirmed via `SHOW GRANTS` that only `mismo` access remains, and `hbm2ddl.auto=validate` passes cleanly afterward. Worth checking for the same stray grant on any other shared local MySQL account before assuming `validate` will behave on a different machine.
- **Database and app account renamed** (2026-08-19, before any production instance exists, so no migration path was needed): `matching_validation` → `mismo`, `mv_web` → `mismo_web` — the old names were legacy from the pre-v2 app. Done via `CREATE DATABASE mismo` + `mysqldump | mysql` copy + row-count verification, then `DROP DATABASE matching_validation`, `RENAME USER`, and re-granting on `mismo.*` only. That re-grant also fully cleared a dangling `GRANT USAGE ... ON aart.* WITH GRANT OPTION` remnant left over from the earlier `REVOKE` above (the original revoke had cleared the privileges but not the `GRANT OPTION` flag itself) — `SHOW GRANTS FOR 'mismo_web'@'localhost'` now shows only `mismo.*`. Every `matching_validation`/`mv_web` reference in this repo, the refresh scripts, and the local backup files (renamed too, e.g. `mismo20260819.sql`) was updated to match; `docs/data-model.md` was deliberately left alone since it's a frozen historical snapshot of the actual v1 system, which really was named `matching_validation`.
- **`WITH GRANT OPTION` dropped** from `mismo_web` (both the local account and `create-user.sql`'s template) — a plain app account never needed the ability to grant its own privileges to others; least-privilege from the start since production doesn't exist yet.
- **`mismo.external.url` moved from a JVM system property to `app_setting`** (2026-08-19) — it started out as deploy-time config (`-Dmismo.external.url`, matching a precedent from the sibling "Clear" app) but that meant changing it on production required a JVM-flag edit and a Tomcat restart, unlike `hub.external.url` next to it which was already a live-editable database row. `HubClientSupport` now reads both the same way (`loadSetting(key, fallback)`, no caching beyond the built `InteropHubClient` object itself, rebuilt automatically whenever either stored value changes). The `MISMO_EXTERNAL_URL` static field and the `-D` flag are gone entirely — nothing to set at the JVM/Tomcat level for this anymore. The new `app_setting` row was applied directly to local dev (not via `unapplied_updates.sql` — production is bootstrapping from a full dump rather than replaying migrations for this first release, so there's nothing to "release" here; the row is just part of the dump, with its production value appended directly to `mismo20260819.sql` alongside the `hub.external.url` statement). `local_database_refresh.sql` resets it back to `http://localhost:8080/mismo` on a local refresh, same shape as `hub.external.url`.

**Not done — needs your input:**
- `MISMO_DB_USER`/`MISMO_DB_PASSWORD` are set at **User scope** on this machine (`mismo_web` / the existing password, unchanged — not rotated, since rotating it also means changing the actual MySQL account's password in lockstep, a deliberate separate step rather than a side effect of this refactor). Setting them at **Machine scope failed** — "Requested registry access is not allowed," this session isn't elevated. User-scope variables aren't visible to a process started as a Windows Service (which is how the local Tomcat instance appears to be running, based on an earlier session's note that it couldn't be stopped/started from here either) — a service reads its environment once, at its own start, from the Machine scope (or from environment configured specifically for that service). Two ways to finish this, your call: run `[Environment]::SetEnvironmentVariable('MISMO_DB_USER','mismo_web','Machine')` (and the password) from an elevated PowerShell yourself, or set them directly on the Tomcat service's own environment via its service-configuration utility (e.g. `tomcat9w.exe`'s "Java" tab) — the second is generally the more robust option for a service specifically, rather than relying on machine-wide variables. Either way, **the Tomcat service needs a restart** afterward to pick up the new environment — variables set after a process starts aren't retroactively visible to it.
